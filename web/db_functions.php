...
function sendMessageThroughGCM($registration_ids, $message) {
	...
	define("GOOGLE_API_KEY", "AIzaSyD2TpBIlbAdT0F2y-eWIUwOVq2Y_neaVpM");
	$headers = array(
...
class DB_FUNCTIONS {
  ...
  public function getAllRegIds() {
	...
	public function getBusPosition($id, $row, $number) {
	  $query = sprintf("SELECT COUNT(*)+1 FROM buses WHERE _id<(SELECT _id FROM buses WHERE school_id='%1\$s' AND row='%2\$s' AND number='%3\$s') AND school_id='%1\$s' AND row='%2\$s'", mysql_real_escape_string($id), mysql_real_escape_string($row), mysql_real_escape_string($number));
